...
from ConfigLoader_MHA import logger, ss_size, vocab_size, config_model, path_parser
from MultiHeadAttentionModule import MultiHeadAttention
...
class Model:
    ...
    def _build_temporal_att(self):
        """
        Multi-head temporal attention for movement prediction
        g: batch_size * max_n_days * g_size
        g_T: batch_size * g_size
        """
        with tf.name_scope('tda'):
            with tf.variable_scope('tda'):
                # Initialize temporal multi-head attention
                temporal_mha = MultiHeadAttention(
                    num_heads=5,  # Match MSIN's num_heads 
                    d_model=self.g_size,
                    scope_name="temporal_mha",
                    dropout=0.1,
                    temperature=1.0,
                    use_layer_norm=True,
                    use_residual=True
                ...
                # Prepare query from g_T (current state)
                g_T_expanded = tf.expand_dims(self.g_T, axis=1)  # [batch_size, 1, g_size]
                
                # Create attention mask
                def get_none_mask():
                    return tf.zeros_like(self.mask_aux_trading_days, dtype=tf.float32)
                
                def get_trading_mask():
                    return tf.cast(self.mask_aux_trading_days, tf.float32)
                
                attention_mask = tf.cond(
                    tf.equal(tf.size(self.mask_aux_trading_days), 0),
                    get_none_mask,
                    get_trading_mask
                )
                # Apply temporal multi-head attention
                # Use g_T as query, g as keys and values
                temporal_context, attention_weights = temporal_mha(
                    q=g_T_expanded,  # [batch_size, 1, g_size]
                    k=self.g,        # [batch_size, max_n_days, g_size]
                    v=self.g,        # [batch_size, max_n_days, g_size]
                    mask= attention_mask,
                    training=self.is_training_phase
                )
                
                # Store attention weights for visualization
                self.v_stared = attention_weights  # [batch_size, max_n_days]
                
                # Select appropriate context based on daily_att flag
                def get_movement_context():
                    y_reshaped = tf.reshape(self.y, tf.shape(self.g)) #tf.transpose(self.y, [0, 2, 1])  # [batch_size, y_size, max_n_days]
                    return y_reshaped  # [batch_size, max_n_days, y_size]
            
                def get_hidden_context():
                    return self.g #tf.transpose(self.g, [0, 2, 1])  # [batch_size, g_size, max_n_days]  # [batch_size, max_n_days, g_size]
                
                context = tf.cond(
                    tf.equal(self.daily_att, 'y'),
                ...
                weighted_context = context * v_stared_expanded  # [batch_size, max_n_days, feature_size]
            
                # Sum over time dimension to get context vector
                att_c = tf.reduce_sum(weighted_context, axis=1)  # [batch_size, feature_size]
                
                # Final prediction
                with tf.variable_scope('y_T'):
                    # Reshape temporal context to match other tensors
                    temporal_context = tf.reshape(temporal_context, [tf.shape(temporal_context)[0], -1])  # [batch_size, g_size*num_heads]
                    
                    # Match ranks for all tensors before concatenation
                    att_c = tf.reshape(att_c, [tf.shape(att_c)[0], -1])  # [batch_size, feature_size]
                    g_T_flat = tf.reshape(self.g_T, [tf.shape(self.g_T)[0], -1])  # [batch_size, g_size]
                    
                    # Concatenate all features
                    combined_features = tf.concat(
                        [att_c, temporal_context, g_T_flat],
                        axis=1
                    )  # [batch_size, feature_size + g_size*num_heads + g_size]
                    
                    # Make final prediction
                    self.y_T = self._linear(
                        combined_features,
                        self.y_size,
                        'softmax'
                    )  # [batch_size, y_size]
    ...
    def _linear(self, args, output_size, activation=None, use_bias=True, use_bn=False):
        """
        Linear projection with shape checking
        """
        ...
        for arg in args:
            ...
            if shape[-1] is not None:  # Static shape available
                total_arg_size += shape[-1]
            else:  # Handle dynamic shape
                dynamic_shape = tf.shape(arg)[-1]
                total_arg_size += tf.cast(dynamic_shape, tf.int32)

        # Create variables
        with tf.variable_scope(tf.get_variable_scope()):
            # Create weight matrix using static shape
            try:
                ...
                # Handle case where total_arg_size is dynamic
                weight_shape = tf.stack([total_arg_size, output_size])
                weight = tf.get_variable(
                    'weight',
                    shape=None,  # Shape will be set dynamically
                    dtype=tf.float32,
                    initializer=lambda: self.initializer(weight_shape)
                )
            ...
            if len(args) == 1:
                ...
                # Reshape each arg to 2D before concatenation
                reshaped_args = [tf.reshape(arg, [-1, tf.shape(arg)[-1]]) for arg in args]
                flat_x = tf.concat(reshaped_args, axis=1)

            # Perform matrix multiplication
            res = tf.matmul(flat_x, weight)
            ...
            # Reshape output
            batch_shape = tf.shape(args[0])[:-1]
            output_shape = tf.concat([batch_shape, [output_size]], 0)
