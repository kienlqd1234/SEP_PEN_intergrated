...
from sep_main.explain_module.agents import PredictReflectAgent
from sep_main.explain_module.util import summarize_trial
from VoSExplainModule import VoSExplainModule
...
class Model:
    ...
    def __init__(self, config_path=None):
        # If config_path is provided, update the ConfigLoader
        if config_path:
            from ConfigLoader import update_config
        ...
        # Initialize VoSExplainModule
        self.vos_explain = VoSExplainModule(self, config_model)
    ...
    def _build_mie(self):
        ...
        with tf.name_scope('mie'):
            ...
            if self.variant_type == 'tech':
                ...
                # Store VoS components for explanation
                self.vos_components = {
                    'price_features': self.price,
                    'text_features': self.msg_embed,
                    'attention_weights': self.P
                }
    ...
    def _build_temporal_att(self):
        ...
        with tf.name_scope('tda'):
            with tf.variable_scope('tda'):
                ...
                # Generate explanation for current prediction
                self.explanation_agent, self.explanation = self.vos_explain.generate_explanation(
                    vos=self.g,  # Use generated VoS
                    price_movement=self.y_T_,  # Ground truth
                    text_features=self.msg_embed  # Text features
                )

                # Store attention weights for explanation
                self.vos_attention = {
                    'temporal_weights': self.v_stared,
                    'feature_importance': v_i,
                    'context_vector': att_c
                }
    ...
    def _create_generative_ata(self):
        ...
        with tf.name_scope('ata'):
            with tf.variable_scope('ata'):
                ...
                self.reflection = self.vos_explain.reflect_and_improve(
                    agent=self.explanation_agent,
                    prediction=self.y_T,
                    ground_truth=self.y_T_
                )
                
                # Compute automated metrics (replacing human evaluation)
                automated_metrics = self.compute_automated_metrics()
                
                # Add reflection loss and automated metrics to total loss
                if self.reflection is not None:
                    reflection_loss = tf.reduce_mean(
                        tf.square(self.reflection['proposed_improvements'] - self.y_T)
                    )
                    self.loss += 0.1 * reflection_loss  # Weight factor for reflection loss
                
                # Add loss components based on automated metrics (replacing human evaluation components)
                # These replace RoR, RTT, and Fleiss' Kappa scores that required human experts
                if 'automated_ror' in automated_metrics:
                    # Encourage high RoR (attention focused on most relevant texts)
                    self.loss += 0.05 * (1.0 - automated_metrics['automated_ror'])
                
                if 'automated_rtt' in automated_metrics:
                    # Encourage high RTT (focused temporal attention)
                    self.loss += 0.05 * (1.0 - automated_metrics['automated_rtt']) 
                
                if 'self_consistency' in automated_metrics:
                    # Encourage high self-consistency (replacing Kappa)
                    self.loss += 0.05 * (1.0 - automated_metrics['self_consistency'])
                    
                # Store computed metrics for later access
    ...
    def assemble_graph(self):
        ...
        with tf.device('/device:GPU:0'):
            ...
            # VoS explanation components are now initialized in _build_temporal_att
            # self.vos_explain = VoSExplainModule(self, config_model)
            # self.explanation = None
            # self.reflection = None
    ...
    def get_vos_explanation(self):
        """
        Returns current VoS explanation and components
        """
        return {
            'vos_components': self.vos_components,
            'attention_weights': self.vos_attention,
            'explanation': self.explanation,
            'reflection': self.reflection
        }

    def compute_automated_metrics(self):
        """
        Compute automated metrics that replace human evaluation:
        - Automated Ratio of Relevance (RoR): Calculated using VoS weights
        - Automated Ratio of Top Two (RTT): Ratio of top 2 attention weights
        - Self-consistency score: Replaces Fleiss' Kappa by measuring consistency
          between multiple explanations generated for the same prediction
        ...
        # Calculate automated RoR using VoS weights
        if hasattr(self, 'P') and self.P is not None:
            # Get top text based on attention weights
            top_indices = tf.nn.top_k(tf.reduce_max(self.P, axis=1), k=2).indices
            # Calculate ratio of top attention weights to total
            top_weights_sum = tf.reduce_sum(tf.gather(self.P, top_indices, axis=2))
            total_weights = tf.reduce_sum(self.P)
            metrics['automated_ror'] = top_weights_sum / (total_weights + 1e-10)
        
        # Calculate RTT from VoS weights
        if hasattr(self, 'v_stared') and self.v_stared is not None:
            # Get top 2 temporal attention weights
            values, _ = tf.nn.top_k(self.v_stared, k=2)
            # Calculate ratio of top 2 to total
            top_two_sum = tf.reduce_sum(values, axis=1)
            total_sum = tf.reduce_sum(self.v_stared, axis=1)
            metrics['automated_rtt'] = top_two_sum / (total_sum + 1e-10)
        
        # Self-consistency score (replacing Fleiss' Kappa)
        if hasattr(self, 'explanation') and hasattr(self, 'reflection'):
            # A non-zero reflection score indicates disagreement with original explanation
            # We use this as an inverse measure of self-consistency
            metrics['self_consistency'] = tf.constant(1.0)
            if self.reflection is not None:
                # Lower score when reflection suggests improvements
                metrics['self_consistency'] = tf.constant(0.5)
