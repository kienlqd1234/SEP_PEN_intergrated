...
import numpy as np
from ConfigLoader_MHA import attention_config
...
class MSINCell(object):        
    ...
    def __init__(self, input_size, num_units, v_size, max_n_msgs, forget_bias=1.0):
        ...
        self.num_heads = 5  # Number of attention heads
        self.mha = MultiHeadAttention(num_heads=self.num_heads, d_model= num_units, scope_name="text_self_attention", dropout= attention_config['dropout'], use_layer_norm=attention_config['use_layer_norm'], use_residual=attention_config['use_residual']) 
        
        # Text-price cross attention
        self.cross_attention = MultiHeadAttention(num_heads=self.num_heads, d_model=num_units, scope_name="text_price_cross_attention", dropout= attention_config['dropout'], use_layer_norm=attention_config['use_layer_norm'], use_residual=attention_config['use_residual'])
    ...
    def call(self, X, S, state):
        ...
        # Multi-head self attention on text
        text_attention, _ = self.mha(S, S, S)  # Self attention on text
        
        # Cross attention between text and market data
        X_proj = tf.layers.dense(X, self._num_units, name='market_projection')  # Project from 3 to num_units
        X_expanded = tf.expand_dims(X_proj, axis=1)  # Add sequence dimension
    
        #X_expanded = tf.expand_dims(X, axis=1)  # Add sequence dimension
        cross_context, cross_weights = self.cross_attention(
            q=X_expanded,  # Market data as query
            k=text_attention,  # Text as key
            v=text_attention   # Text as value
        )
        
        # Ensure cross_weights have shape [batch_size, max_n_msgs]
        cross_weights = tf.reshape(cross_weights, [-1, self.max_n_msgs])
        
        # Squeeze sequence dimension
        text = tf.squeeze(cross_context, axis=1)
        ...
        return H, cross_weights, new_state
